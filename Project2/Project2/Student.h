...
public:
	...
	Student(int newID, const std::string& newFirstName,
		const std::string& newLastName, bool paid, 
		const std::multimap<Course, char>& courses) : 
		studentID(newID), firstName(newFirstName), 
		lastName(newLastName), 
		numberOfCourses(static_cast<int>(courses.size())), 
		tuitionWasPaid(paid), coursesCompleted(courses) {}
	...
	void getName(std::string& newFirstName, 
